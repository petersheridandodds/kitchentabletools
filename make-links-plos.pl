...
foreach $line (<FILE>)
{
	...
	if ($line =~ m/\\includegraphics\[.*?\]\{(.*?)\}/) {
		...
		`rm $folder/$prefix$figfile.ps`;
                if (! -f "$folder/$prefix$figfile.tiff" ) { 
        ...
        # if ($line =~ m/\\includetable\{(.*?)\}/) {
        #     print "found line of table";
	#     $tablefile = $1;
	...
	#     $tablefile =~ s/^.*\///;
        #     # if ($j<10) {$codenum = "0$j";} else {$codenum = $j;}
	#     $prefix = "table".$k."_";
	#     `cp $fulltable $folder/$prefix$tablefile`;
        #     print "commenting out table";
        #     $line =~ s/(\\includetable.*?)\{(.*?)\}/%% $1\{$prefix$tablefile\}/;
 	#     $k = $k + 1;
	# }
        if ($caption) {
            $caption = 0;
